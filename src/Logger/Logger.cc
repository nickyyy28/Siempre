...
#undef XX
    ...
    LogEvent::LogEvent(std::string name, std::shared_ptr<Logger> logger, const char *filename, uint32_t line,
                       uint64_t elapse,
                       uint32_t thread_id, const std::string thread_name, uint32_t fiber_id, uint64_t time,
...
#undef XX
    ...
    void ThreadIdFormatItem::format(std::ostream &os, std::shared_ptr<Logger> logger, LogLevel::Level level,
                                    LogEvent::ptr event) {
        os << event.get()->getThreadId();
    }

    void TabFormatItem::format(std::ostream &os, std::shared_ptr<Logger> logger, LogLevel::Level level,
                               LogEvent::ptr event) {
        os << "\t";
    }

    void ThreadNameFormatItem::format(std::ostream &os, std::shared_ptr<Logger> logger, LogLevel::Level level,
                                      LogEvent::ptr event) {
        //printf("len : %d\n", event->getThreadName().size());
        os << event.get()->getThreadName().c_str();
    }

    void FiberIDFormatItem::format(std::ostream &os, std::shared_ptr<Logger> logger, LogLevel::Level level,
                                   LogEvent::ptr event) {
        os << event.get()->getFiberId();
    }

    void LevelFormatItem::format(std::ostream &os, std::shared_ptr<Logger> logger, LogLevel::Level level,
                                 LogEvent::ptr event) {
        os << LogLevel::getStringByLevel(level);
    }

    void FileNameFormatItem::format(std::ostream &os, std::shared_ptr<Logger> logger, LogLevel::Level level,
                                    LogEvent::ptr event) {
        os << "\"" << event.get()->getFile() << "\"";
    }

    void LineFormatItem::format(std::ostream &os, std::shared_ptr<Logger> logger, LogLevel::Level level,
                                LogEvent::ptr event) {
        os << event.get()->getLine();
    }

    void MessageFormatItem::format(std::ostream &os, std::shared_ptr<Logger> logger, LogLevel::Level level,
                                   LogEvent::ptr event) {
        os << "\"" << event.get()->getContent() << "\"";
    }

    void NewLineFormatItem::format(std::ostream &os, std::shared_ptr<Logger> logger, LogLevel::Level level,
                                   LogEvent::ptr event) {
        os << "\r\n";
    }

    void NameFormatItem::format(std::ostream &os, std::shared_ptr<Logger> logger, LogLevel::Level level,
                                LogEvent::ptr event) {
        os << event.get()->getEventName();
    }

    void ElapseFormatItem::format(std::ostream &os, std::shared_ptr<Logger> logger, LogLevel::Level level,
                                  LogEvent::ptr event) {
        ...
        if (level >= this->m_level) {
            switch (level) {
                case LogLevel::DEBUG:
                    ...
                    break;
                case LogLevel::INFO:
                    std::cout << L_GREEN << (this->m_formatter).get()->format(logger, level, event) << NONE
                              << std::endl;
                    break;
                case LogLevel::WARN:
                    std::cout << L_YELLOW << (this->m_formatter).get()->format(logger, level, event) << NONE
                              << std::endl;
                    break;
                case LogLevel::ERROR:
